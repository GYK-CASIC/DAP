...
def run_web_ui() -> None:
    ...
    create_ui().queue().launch(share=False, server_name=server_name, inbrowser=True)
    

def run_web_demo() -> None:
    gradio_share = os.environ.get("GRADIO_SHARE", "0").lower() in ["true", "1"]
    server_name = os.environ.get("GRADIO_SERVER_NAME", "127.0.0.1")
    create_web_demo().queue().launch(share=False, server_name=server_name, inbrowser=True)
